...
//////
	...
	static FORCEINLINE int     atomic_cas32_acquire(atomic32_t* dst, int32_t val, int32_t ref) { return ( _InterlockedCompareExchange(dst, val, ref) == ref) ? 1 : 0; }
	...
	static FORCEINLINE void*   atomic_exchange_ptr_acquire(atomicptr_t* dst, void* val) { return (void*) _InterlockedExchangePointer((void* volatile*)dst, val); }
	...
	static FORCEINLINE int     atomic_bit_test_and_set( atomic32_t* val, int n ) { return _interlockedbittestandset( ( volatile long* ) val, n ); }
	static FORCEINLINE void    _rpmalloc_yield() { _mm_pause(); }
#else
	#define EXPECTED(x)   __builtin_expect((x), 1)
	#define UNEXPECTED(x) __builtin_expect((x), 0)
	static FORCEINLINE int     atomic_bit_test_and_set(atomic32_t* val, int n) { int out; asm volatile( "lock btsl %2, %1" : "=@ccc" ( out ), "+m" ( *val ) : "Jr" ( n ) ); return out; }
	static FORCEINLINE void    _rpmalloc_yield() { __asm__ volatile( "pause" ::: "memory" ); }
#endif
...
get_thread_heap_raw(void) {
	return ( heap_t* ) rpmalloc_get_tls();
}
...
//! Fast thread ID
static FORCEINLINE uintptr_t get_thread_id(void) {
	return rpmalloc_get_thread_id();
}

//! Set the current thread heap
static void
set_thread_heap(heap_t* heap) {
	rpmalloc_set_tls( heap );
	if (heap)
...
_rpmalloc_mmap(size_t size, size_t* offset) {
	...
	return rpmalloc_mmap_os(size, offset);
}
...
_rpmalloc_unmap(void* address, size_t size, size_t offset, size_t release) {
	...
	rpmalloc_unmap_os(address, size, offset, release);
}
...
_rpmalloc_span_map(heap_t* heap, size_t span_count) {
	...
	if (use_global_reserve) {
		// If huge pages, make sure only one thread maps more memory to avoid bloat <-- Fuck this.
		//lock_irq = _rpmalloc_acquire( &_memory_global_lock );
	...
	if ( lock_irq >= 0 )
		_rpmalloc_release(&_memory_global_lock, lock_irq );
	return span;
}
...
_rpmalloc_global_cache_finalize(global_cache_t* cache) {
	...
	int lock_irq = _rpmalloc_acquire( &cache->lock );
	...
	_rpmalloc_release( &cache->lock, lock_irq );
}
...
_rpmalloc_global_cache_extract_spans(span_t** span, size_t span_count, size_t count) {
	...
	int lock_irq = _rpmalloc_try_acquire( &cache->lock );
	if ( lock_irq < 0 ) return 0;
	...
	for (size_t ispan = 0; ispan < extract_count; ++ispan) {
		rpmalloc_assert(span[ispan]->span_count == span_count, "Invalid.");
	}
#endif
...
_rpmalloc_heap_allocate(int first_class) {
	...
	int lock_irq = _rpmalloc_acquire( &_memory_global_lock );
...
_rpmalloc_heap_release(void* heapptr, int first_class, int release_cache) {
	...
	if ( lock_irq >= 0 ) _rpmalloc_release( &_memory_global_lock, lock_irq );
}
